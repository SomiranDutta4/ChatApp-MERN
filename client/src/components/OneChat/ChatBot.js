...
const ChatBot = ({ setloadAll, setSingleChat }) => {
    ...
    const { User, setShowingBot, clickedChat, setClicked, setSending, setUser, setChats, messageLoading, setMsgLoadig, URL } = useContext(AppContext)

    const getTheAi = async () => {
        // setClicked('')
        const AiUrl = URL + `/chat/ai/bot/?token=${User.token}`
    ...
    const sendMessage = async () => {
        ...
        setMessage('')
        scrollIt()
        const MessageUrl = URL + `/message/ai/bot/?token=${User.token}`
