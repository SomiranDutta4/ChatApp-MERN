...
module.exports.fetchAllChats = async function (req, res) {
    try {
        ...
        chats = chats.filter(chat => {
            const deletionEntry = chat.deletedFor.find(entry => entry.user.toString() === userId);

            if (!deletionEntry) return true;

            const deletedAt = new Date(deletionEntry.at);
            const messageTime = chat.latestMessage?.createdAt ? new Date(chat.latestMessage.createdAt) : null;

            return messageTime && messageTime > deletedAt;
        });

        return res.status(200).json({
            chats: chats,
            message: chats.length > 0 ? 'Fetched successfully' : 'No chats found'
        });
    } catch (error) {
...
module.exports.leaveChat = async function (req, res) {
    const { userId, chatId } = req.body;

    if (!userId || !chatId) {
        return res.status(400).json({ message: 'userId and chatId required' });
    }
    try {
...
module.exports.deleteChat = async function (req, res) {
    try {
        ...
        const alreadyDeleted = chat.deletedFor?.find(entry => entry.user.toString() === userId.toString());

        if (!alreadyDeleted) {
            chat.deletedFor = [...(chat.deletedFor || []), { user: userId, at: new Date() }];
            await chat.save();
        }
